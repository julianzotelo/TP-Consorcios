...
--valido que se haya creado LA funcion
...
go
-- =============================================
-- Script de testing para SP_Generar_Expensas_Completo
-- =============================================


-- Par�metros de prueba
DECLARE @ID_consorcio INT = 1;           -- Cambiar por un consorcio que se desee generar
DECLARE @periodo CHAR(20) = 'marzo-2025';  -- Cambiar por el per�odo de prueba que uno quiera


-- Ejecutar el SP
EXEC SP_Generar_Expensas_Completo
    @ID_consorcio = @ID_consorcio,
    @periodo = @periodo;

    SELECT dbo.fn_CalcularVencimiento('marzo') AS Vencimiento;
...
SELECT * FROM Estado_cuenta_prorrateo WHERE ID_consorcio = @ID_consorcio AND periodo = LOWER(@periodo);
	...
	--select * from  Estado_cuenta_prorrateo;
	--select * from  Factura;
	--select * from  Detalle_EstadoFinanciero;
	--select * from  Estado_financiero;
	--select * from  TipoDetalleFinanciero;
